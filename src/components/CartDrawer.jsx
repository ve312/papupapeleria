...
export default function CartDrawer({ open, onClose, goToCart }) {
  const { items, subtotal, inc, dec, remove, freeShipProgress, remainingForFreeShip, FREE_SHIP_THRESHOLD } = useCart();
  ...
  return (
    <div className={`fixed inset-0 z-50 ${open ? 'pointer-events-auto' : 'pointer-events-none'}`}>
      ...
      />
      {/* Panel */}
      <aside className={`absolute right-0 top-0 h-full w-full sm:w-[420px] bg-white shadow-2xl flex flex-col transition-transform ${open ? 'translate-x-0' : 'translate-x-full'}`}>
        {/* Header */}
              ...
              <div key={item.id} className="flex gap-4">
                ...
                <div className="flex-1">
                  ...
                  <div className="mt-2 flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <button onClick={() => dec(item.id)} className="p-1 rounded border hover:bg-gray-50"><Minus className="w-4 h-4" /></button>
                      <input
                        ...
                        readOnly
                      />
                      <button onClick={() => inc(item.id)} className="p-1 rounded border hover:bg-gray-50"><Plus className="w-4 h-4" /></button>
                    </div>
                    <div className="text-right">
                      <div className="text-sm text-gray-500">{format(item.price)} c/u</div>
                      <div className="font-semibold text-gray-800">{format(item.price * item.qty)}</div>
                    </div>
                  </div>
                </div>
                <button onClick={() => remove(item.id)} className="p-2 h-10 self-start rounded hover:bg-gray-100">
        ...
        <div className="border-t p-5 space-y-3">
          ...
          <p className="text-xs text-gray-500">Impuestos y envío se calculan al finalizar la compra</p>
          <div className="flex gap-3">
            ...
            <button
              className="flex-1 py-3 rounded-xl font-semibold text-white bg-gradient-to-r from-orange-500 to-red-500 hover:shadow-lg"
            >
              Ir a pagar
            </button>
          </div>
